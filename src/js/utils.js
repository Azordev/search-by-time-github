...
export const createSearchFilter = (formData) => {
  const { user, repository, typeOfSearch, condition, date } = formData;
...
const changeAnchorLink = (
  anchorElement,
  { user, repository, typeOfSearch, date }
) => {
  if (anchorElement) {
    anchorElement.href = `https://github.com/${user}/${repository}/issues?q=${encodeURI(
      `${typeOfSearch} ${date}`
    )}`;
...
export const validateForm = () => {
  ...
  fields.forEach((field) => {
    ...
    if (fielValid[idField].required && !value) {
      ...
      spanError.innerHTML = 'Please enter the required field';
      isValid = false;
    } else {
